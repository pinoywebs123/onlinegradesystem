        ...
        <div class="row">
            <div class="panel panel-info">
                <div class="panel-heading">
                    <h3 class="text-center">Edit Class Record</h3>
                </div>
                <div class="panel-body">
                    @foreach($editclass as $editclass)
                   <div class="col-md-5 col-md-offset-3">
                       <form action="{{route('update_class', ['student_id'=> $editclass->user_id])}}" method="post">
                    {{csrf_field()}}
                   
                    <div class="form-group">
                        <label for="student_id">Student ID</label>
                        <input type="text" name="student_id" class="form-control" value="{{$editclass->user_id}}">
                    </div>
                    <div class="form-group">
                        <label for="lname">Last Name</label>
                        <input type="text" name="lname" class="form-control" value="{{$editclass->last_name}}">
                    </div>
                    <div class="form-group">
                        <label for="fname">First Name</label>
                        <input type="text" name="fname" class="form-control" value="{{$editclass->first_name}}">
                    </div>
                    <div class="form-group">
                        <label for="mname">Middle Name</label>
                        <input type="text" name="mname" class="form-control" value="{{$editclass->middle_name}}">
                    </div>
                    <div class="form-group">
                        <label for="level">Grade Level</label>
                        <input type="text" name="level" class="form-control" value="{{$editclass->level}}">
                    </div>
                    <div class="form-group">
                        <label for="room">Room Number</label>
                        <input type="text" name="room" class="form-control" value="{{$editclass->room_number}}">
                    </div>
                    @endforeach
