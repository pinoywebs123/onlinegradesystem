          ...
          @if(Session::has('update'))
           ...
           @endif
            <table class="table">
                        ...
                        <tbody>
                            <form action="{{route('edit_student_editHandle4',['student_id'=>$student->user_id, 'level'=> $student->level ])}}" method="post">
                                <tr>
                                    <td>Final</td>
                                        <?php $num = 0;?>
                                        @foreach($finalgrade as $grade)
                                        <td><input type="text" name="{{$num = $num+1}}fs" size="4" required="" value="{{$grade->grade}}"></td>
